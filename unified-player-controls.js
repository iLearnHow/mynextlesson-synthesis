...
class UnifiedPlayerControls {
    constructor() {
        ...
        this._elementOverlayVisible = false;
    ...
    setupBottomControls() {
        ...
        // { } button – default: Show-All element map overlay; with modifier: open Code & Variants
        const codeBtn = document.getElementById('code-viewer');
        codeBtn.addEventListener('click', (ev) => {
            // Hold Shift/Alt/Meta to open the previous Code & Variants popup
            if (ev && (ev.shiftKey || ev.altKey || ev.metaKey || ev.ctrlKey)) {
                this.openPopup('code', 'Code & Variants', this.generateCodeContent());
                return;
            }
            this.toggleElementMapOverlay();
        });
    }
    ...
    togglePlayback() {
        ...
        if (this.currentState.isPlaying) {
            ...
            // No speechSynthesis - using unified TTS only
        }
    ...
    /**
     * Toggle on-screen labels for every visible, named element
     * This is the "show all" mode bound to the { } button
     */
    toggleElementMapOverlay(){
        if (this._elementOverlayVisible) {
            this.destroyElementMapOverlay();
            return;
        }
        this.buildElementMapOverlay();
    }

    /** Build overlay DOM with boxes and labels for all visible, named nodes */
    buildElementMapOverlay(){
        try {
            // Remove any existing instance
            this.destroyElementMapOverlay();

            const overlay = document.createElement('div');
            overlay.id = 'element-map-overlay';
            ...
            legend.style.left = '14px';
            legend.style.top = '14px';
            legend.style.zIndex = '5001';
            legend.style.pointerEvents = 'auto';
            legend.style.padding = '10px 12px';
            legend.style.borderRadius = '10px';
            legend.style.fontFamily = "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', sans-serif";
            legend.style.fontSize = '13px';
            legend.style.color = '#1d1d1f';
            legend.style.background = 'rgba(255,255,255,0.92)';
            legend.style.border = '1px solid rgba(0,0,0,0.12)';
            legend.style.boxShadow = '0 6px 18px rgba(0,0,0,0.16)';
            legend.innerHTML = `
                <div style="display:flex;align-items:center;gap:8px;">
                    <span style="font-weight:700;">Show All</span>
                    <span style="opacity:.7;">Click labels to copy selectors · Press Esc or { } again to exit</span>
                </div>`;
            overlay.appendChild(legend);

            const elements = this._collectNamedElements();
            elements.forEach(({ el, id, rect, selector }) => {
                // Bounding box
                ...
                box.style.border = '1px dashed rgba(0,122,255,0.7)';
                box.style.background = 'rgba(0,122,255,0.05)';
                ...
                label.style.top = Math.round(Math.max(0, rect.top - 20)) + 'px';
                label.style.transform = 'translateY(-6px)';
                label.style.maxWidth = '48vw';
                label.style.whiteSpace = 'nowrap';
                label.style.overflow = 'hidden';
                label.style.textOverflow = 'ellipsis';
                label.style.padding = '4px 8px';
                label.style.borderRadius = '999px';
                label.style.fontFamily = "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', sans-serif";
                label.style.fontSize = '12px';
                label.style.fontWeight = '600';
                label.style.color = '#1d1d1f';
                label.style.background = 'rgba(255,255,255,0.95)';
                label.style.border = '1px solid rgba(0,0,0,0.12)';
                label.style.boxShadow = '0 6px 16px rgba(0,0,0,0.14)';
                label.style.pointerEvents = 'auto';
                label.style.cursor = 'copy';
                label.title = 'Click to copy selector';

                label.addEventListener('click', (e)=>{
                    e.preventDefault(); e.stopPropagation();
                    try {
                        navigator.clipboard.writeText(selector);
                        label.style.background = 'rgba(84,199,236,0.25)';
                        setTimeout(()=>{ label.style.background = 'rgba(255,255,255,0.95)'; }, 450);
                    } catch {}
                });
            ...
            const onEsc = (ev)=>{ if (ev.key === 'Escape') { this.destroyElementMapOverlay(); } };
            document.addEventListener('keydown', onEsc, { once: true });

            this._elementOverlay = overlay;
            this._elementOverlayVisible = true;
            console.log(`🎯 Element map overlay: ${elements.length} items`);
        } catch (e) {
            console.warn('element map overlay failed', e);
        }
    }

    /** Remove overlay */
    destroyElementMapOverlay(){
        try {
            if (this._elementOverlay && this._elementOverlay.parentNode) {
                this._elementOverlay.parentNode.removeChild(this._elementOverlay);
            }
        } catch {}
        this._elementOverlay = null;
        this._elementOverlayVisible = false;
    ...
    /**
     * Collect visible, named elements with good selectors
     */
    _collectNamedElements(){
        const results = [];
        try {
            const all = Array.from(document.querySelectorAll('*'));
            const skipIds = new Set(['element-map-overlay']);
            for (const el of all){
                ...
                // Require an identifying selector
                ...
                let selector = '';
                if (id) selector = id;
                else if (dataName) selector = `[data-name="${dataName}"]`;
                else if (dataRole) selector = `[data-role="${dataRole}"]`;
                else if (classes.length && classes[0].length > 2) selector = `${el.tagName.toLowerCase()}.${classes[0]}`;
                else continue;

                results.push({ el, id: el.id||null, rect, selector });
            }
        } catch {}
        return results;
    }
    ...
    async tryPhonemeTest(){
        try {
            ...
            // Resolve audio URL from clips map if available (prefer mp3)
            let audioUrl = `/phoneme_data/audio/${clip}.mp3`;
            try {
    ...
    async generateAndPlayNarrationForCurrentSlide(){
        try {
            ...
            if (!window.__forceSpeech && tts && typeof tts.generateAudio === 'function') {
                ...
                // No fallback - require proper TTS
