...
class UniversalLessonPlayer {
    constructor() {
        ...
        this.fullFrameVisemesEnabled = false;
        // Viseme mapping
...
The system is ready for real curriculum integration!`,
    ...
    async speak(text, avatar = null) {
        ...
        // Browser speech synthesis fallback on preview builds
        try {
            if (window.__forceSpeech && 'speechSynthesis' in window) {
                const u = new SpeechSynthesisUtterance(narration);
                u.rate = 1.0; window.speechSynthesis.cancel(); window.speechSynthesis.speak(u);
                this.isPlaying = true; this.updatePlayButton();
                return;
            }
        } catch{}
    ...
    _ensureLive2DCompositor(){
        try {
            ...
            // Optionally preload full-frame viseme images
            if (this.fullFrameVisemesEnabled && !this._live2D.full){
                this._live2D.full = { images:{}, last:null };
                const avatar = this.currentVariant.avatar || 'kelly';
                const base = `/production-deploy/assets/avatars/${avatar}/2d/full`;
                const names = this.VISEMES;
                names.forEach(n=>{
                    const img = new Image(); img.crossOrigin='anonymous';
                    // Try png then jpg
                    img.src = `${base}/${avatar}_${n}.png`;
                    img.onerror = ()=>{ try { img.onerror=null; img.src = `${base}/${avatar}_${n}.jpg`; } catch{} };
                    this._live2D.full.images[n] = img;
                });
            }
            ...
            const loop = ()=>{
                ...
                if (this.fullFrameVisemesEnabled && this._live2D.full && this._live2D.full.images[best]){
                    if (this._live2D.full.last !== best) {
                        ...
                        const im = this._live2D.full.images[best];
                        if (bg && im) {
                            const avatar = this.currentVariant.avatar || 'kelly';
                            // Use the exact URL string we loaded to avoid flicker
                            const url = im.src;
